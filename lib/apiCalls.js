...
function fbsList(fbsListYear) {
    ...
    .then(function(data){
        console.log('API called successfully for FBS Teams');
        //console.log('API called successfully. Returned data: ' + data[0].id);
        fs.writeFileSync("./LiveAPIData/FbsTeams_data.json", JSON.stringify(data, null, 2));
        return data;
    }, function(error) {
        console.error(error);
    })
...
function fbsGames(fbsGamesYear//, fbsGamesID
    ...
    .then(function(data){
        console.log('API called successfully for FBS Games');
        fs.writeFileSync("./LiveAPIData/Game_data.json", JSON.stringify(data, null, 2));
    }, function(error) {
        console.error(error);
    })
}
